...
function OrderList() {
    ...
    // 2025-01-01 => 2025/01/01 => 2025년01월01일
    const [startDate, setStartDate] = useState(null) // 시작날짜 (포맷전)
    const [endDate, setEndDate] = useState(null) // 끝날짜 (포맷전)
    ...
    }, [page, formattedStartDate, formattedEndDate, cancelComplete, deleteComplete, dispatch])

    // 주문 취소
    const handleCancleOrder = useCallback((id) => {
        const result = window.confirm('주문을 취소하겠습니까?')
    ...
    const handleDeleteOrder = useCallback((id) => {
        const result = window.confirm('주문을 삭제하겠습니까?')
    ...
    const handleDateFilter = useCallback(() => {
        ...
        setPage(1) // 필터링 시 페이지를 첫번째 페이지로 초기화
    }, [startDate, endDate])
                            ...
                            <Grid xs={12} key={order.id} sx={{ width: '100%' }}>
                                <Card sx={{ display: 'flex', mb: 2, position: 'relative' }}>
                                    ...
                                    <CardContent sx={{ flex: 1 }}>
                                        ...
                                        <Typography variant="body2" color="textSecondary" gutterBottom>
                                            총 주문 금액: {order.Items.map((i) => i.OrderItem.orderPrice)}원
                                        </Typography>
                                    </CardContent>
                                    {order.orderStatus === 'ORDER' ? (
                                        <Button variant="contained" color="info" size="small" sx={{ position: 'absolute', top: 8, right: 8 }} onClick={() => handleCancleOrder(order.id)}>
                                            주문 취소
                                        </Button>
                                    ) : (
