...
module.exports = (server, sessionMiddleware) => {
    ...
    const io = new Server(server, {
        // 소켓에서 별도로 cors 설정: socket.ID와 Express의 cors처리 방식이 다르기 때문
        cors: {
            origin: process.env.FRONTEND_APP_URL, // 클라이언트의 url 허용
            method: ['GET', 'POST'], // 허용된 http 메서드, (put, delete, patch는 차단)
    ...
    io.use((socket, next) => {
        // express의 세션 미들웨어를 Socekt.IO에서 사용할 수 있도록 설정
        sessionMiddleware(socket.request, {}, next)
    })

    // const deserialUser = socket.request.session?.passport?.user

    // 소켓의 연결(connection 이벤트)이 발생하기 직전에 실행되는 미들웨어
    // Passport의 역직렬화 호출 추가
    io.use((socket, next) => {
        // socket.request.session.passport.user에 저장된 사용자 id 확인
        const requestUser = socket.request.session?.passport?.user

        if (requestUser) {
            // passport의 역직렬화 호출
            passport.deserializeUser(requestUser, (err, user) => {
                if (err) return next(err)
                socket.request.user = user // 역직렬화된 사용자 정보를 socket.request 객체에 저장
                next()
            })
        } else {
            // 로그인이 안된 상태에서 소켓에 접속시 소켓 연결 해제(로그인한 사람만 채팅 사용가능)
    ...
    io.on('connection', (socket) => {
        ...
        // 클라이언트에서 'user info' 이벤트 요청 시 사용자 정보 클라이언트에 전송
        ...
        socket.on('chat message', (msg) => {
            // 모든 클라이언트에게 메세지와 사용자 이름 전송
            io.emit('chat message', { user: user?.name, message: msg })
        })

        socket.on('disconnect', () => {
            console.log('사용자 연결 해제:', user?.id) // 연결 해제된 사용자 id 출력
